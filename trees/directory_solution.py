...
class FileSystem:
    ...
    def create_file(self, name, data):
        ...
        """
        file = File(name, data)
        file.parent = self.current_directory
        self.current_directory.children.append(file)
    ...
    def delete_node(self, name):
        ...
        """
        for i, node in enumerate(self.current_directory.children):
            if node.name == name:
    ...
    def list_files(self):
        ...
        """
        for node in sorted(self.get_children(), key=lambda x: x.name):
            # isinstance zistuje, ci je objekt daneho typu (teda ci je instanciou triedy).
            print(f'({"f" if isinstance(node, File) else "d"}) {node.name}')
